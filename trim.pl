...
--trim-type <num>           trim using length (0) or percent (1), default 0, only use in type 1 now
...
while ($p1_header1 = <PAIR1>) {
	...
	if ($type == 0) {
		...
		$p1_flag=1;
                ...
                $length_threshold=$trim_type == 0 ? $length_threshold : (length $p1_seq)*(1-$percent_threshold);
                print "length cut: $length_threshold\n" if $verbose;
		foreach $qual_char (split (//, $p1_qual)) {
			$qualnum = get_quality_num ($qual_char, $qual_type);
                        print "Q$qualnum\($qual_char\) " if $verbose;
			...
			$pos++;
                ...
                print "\n" if $verbose;
                print "p1flag: $p1_flag\tp2flag: $p2_flag\tp1cut: $p1_cut\tp2cut: $p2_cut\n" if $verbose;
